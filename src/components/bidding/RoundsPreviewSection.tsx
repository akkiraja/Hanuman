...
export default function RoundsPreviewSection({ 
  ...
  // Show only last 2-3 completed rounds for preview
  const completedRounds = rounds.filter(round => round.status === 'completed');
  const previewRounds = completedRounds.slice(-3); // Last 3 rounds
  ...
  const renderRoundPreview = (round: BidRound) => {
    ...
    return (
      <TouchableOpacity
        key={round.id}
        style={[styles.roundPreviewItem, Colors.shadow]}
      ...
      <View style={styles.content}>
        {previewRounds.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateText}>No completed rounds yet</Text>
