...
export default function AddMembersScreen({ navigation, route }: AddMembersScreenProps) {
  ...
  useEffect(() => {
    // Set loading state immediately before deferring
    setIsLoading(true);
    
    // Defer heavy contact loading until after first render frame
    // This ensures the loading skeleton appears instantly
  ...
  const loadContactsData = async () => {
    try {
      // Loading state already set in useEffect before deferring
  ...
  // Animated shimmer loading component
  const ShimmerItem = ({ index }: { index: number; key?: number }) => {
    const opacity = useSharedValue(0.3);
    
    useEffect(() => {
      // Start animation with delay based on index for wave effect
    ...
    return (
      <Animated.View style={[styles.loadingItem, animatedStyle]}>
