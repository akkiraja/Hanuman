// @verify_jwt: true
// Last updated: 2025-11-06 - Reduced to 2 SMS events (OTP + group_joined only) to optimize Twilio costs
...
const ALLOWED_SMS_EVENTS = ['group_joined'];
// Note: 'otp_sms' is handled separately (no filtering) - KEPT for authentication
...
interface GroupSMSData {
  ...
  event: 'group_joined';
  // Required params:
  // group_joined: adminName, groupName
  groupName: string; // Required for all events
  adminName?: string; // Required for group_joined
...
async function sendGroupSMS(data: GroupSMSData) {
  try {
    const { groupId, event, groupName, adminName } = data;
    ...
    const eventMessages: { [key: string]: string } = {
      ...
      // Removed: lucky_draw_winner - Cost optimization
      // Removed: bidding_start - Cost optimization
    };
    ...
    // Removed: bidding_start validation - event disabled for cost optimization
    // Removed: lucky_draw_winner validation - event disabled for cost optimization
    if (event === 'group_joined') {
      if (!adminName) {
