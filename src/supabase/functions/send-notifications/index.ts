...
async function sendBidRoundStartNotifications(supabaseClient: any, data: BidRoundStartData) {
  try {
    ...
    // Prepare Hinglish notification messages
    const notifications: NotificationPayload[] = pushTokens.map(tokenData => ({
      to: tokenData.token,
      title: 'Naya Round Shuru Hua 🎉',
      body: `Round ${roundNumber} in ${groupName} abhi abhi start hua hai. Jaldi se bid lagao warna mauka chala jayega!`,
      data: {
...
async function sendBidPlacedNotifications(supabaseClient: any, data: BidPlacedData) {
  try {
    ...
    console.log(`💸 Sending bid placed notifications for group: ${groupId}, bidder: ${bidderName}`);
    ...
    const notifications: NotificationPayload[] = pushTokens.map(tokenData => ({
      to: tokenData.token,
      title: `${bidderName} ne bid lagayi 💸`,
      body: `${bidderName} ne Round ${roundNumber} mein ₹${bidAmount.toLocaleString()} ka dhamaka bid lagaya. Ab tumhari baari hai!`,
      data: {
        type: 'bid_placed',
        groupId,
        groupName,
...
async function sendBidUpdatedNotifications(supabaseClient: any, data: BidUpdatedData) {
  try {
    ...
    const notifications: NotificationPayload[] = pushTokens.map(tokenData => ({
      to: tokenData.token,
      title: `${bidderName} ne bid badal di 🔄`,
      body: `${bidderName} ab nayi chaal chal rahe hai – bid ₹${bidAmount.toLocaleString()} kar di! Tum kya karoge?`,
      data: {
        type: 'bid_updated',
        groupId,
        groupName,
...
async function sendWinnerDeclaredNotifications(supabaseClient: any, data: WinnerDeclaredData) {
  try {
    ...
    const notifications: NotificationPayload[] = pushTokens.map(tokenData => ({
      to: tokenData.token,
      title: 'Winner Mil Gaya 🏆',
      body: `${winnerName} ne Round ${roundNumber} jeet liya ₹${winningAmount.toLocaleString()} ke bid pe. Baki log next time better luck!`,
      data: {
