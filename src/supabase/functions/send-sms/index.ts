...
interface GroupSMSData {
  groupId: string;
  event: 'lucky_draw_start' | 'bidding_start' | 'winner_declared';
  groupName?: string;
  winnerName?: string;
  amount?: number;
  roundNumber?: number;
...
async function sendGroupSMS(data: GroupSMSData) {
  try {
    ...
    const messages = [];
    const eventMessages = {
      lucky_draw_start: '🎰 Bhishi Alert! Admin ne Lucky Draw shuru kar diya hai! App kholo aur live dekhne ka maza lo 🎉',
      bidding_start: '⚡ Bhishi Bidding Live hai! Abhi app kholo aur apna bid lagao 🤑 Jaldi karo warna chance nikal jaayega!',
      winner_declared: `🎉 Bhishi Result: ${winnerName} ne round jeet liya hai ₹${amount?.toLocaleString()} ke saath! Badhai ho 👏 Details app mein dekho 📲`,
    };
