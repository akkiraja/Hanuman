...
export default function GroupDetailScreen({ navigation }: Props) {
  const { currentGroup, joinGroup, conductLuckyDraw, conductLuckyDrawWithWinner, finalizeLuckyDraw, finalizeDraw, selectManualWinner, addMembersToGroup, isLoading, fetchGroups, loadGroupDetails, deleteGroup, leaveGroup, appointCoAdmin, removeCoAdmin } = useChitStore();
  ...
  const handleSpinnerComplete = async (winner: any) => {
    try {
      // NEW: Call finalizeDraw with drawId instead of finalizeLuckyDraw
      if (selectedWinner?.drawId) {
        console.log('🎯 Finalizing draw:', selectedWinner.drawId);
        await finalizeDraw(selectedWinner.drawId);
      }
      
      // Then finalize in group (legacy flow for updating group_members)
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Invite Members CTA Banner - Admin Only */}
        {isGroupAdmin(user?.id, currentGroup) && (
        <TouchableOpacity 
            ...
            <View style={styles.inviteBannerContent}>
              <View style={styles.inviteBannerText}>
                <Text style={styles.inviteBannerTitle}>
                  Invite Members ({currentGroup.currentMembers || 0}/40 added)
                </Text>
                <Text style={styles.inviteBannerSubtitle}>
                  Tap to invite your friends — it only takes a minute.
                </Text>
              </View>
              <View style={styles.inviteBannerButton}>
