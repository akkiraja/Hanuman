...
export default function BiddingGroupDetailScreen({ navigation }: BiddingGroupDetailScreenProps) {
  ...
  if (currentGroup?.id && !isInitialLoading) {
      ...
      // PHASE 3: Deferred Loading - Load user-specific data after interactions complete
      useEffect(() => {
        if (currentGroup?.id && user?.id && !isInitialLoading) {
          ...
          // Defer to after all interactions complete (non-blocking)
          const task = InteractionManager.runAfterInteractions(() => {
            console.log('🔄 Phase 3: Now loading deferred data:', currentGroup.id);
            Promise.all([
              loadUserBids(currentGroup.id, user.id),
              loadBidHistory(currentGroup.id)
            ]).catch(error => {
              console.error('Deferred loading failed:', error);
            });
          });
