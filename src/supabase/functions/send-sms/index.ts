// @verify_jwt: true
// Last updated: 2025-10-17 - Added SMS deduplication (1-min window)
...
async function sendGroupSMS(data: GroupSMSData) {
  try {
    const { groupId, event, groupName, adminName, winnerName, amount, roundNumber, nextDrawDate, drawDate, bidderName, bidAmount, app_link } = data;
    ...
    const messages = [];
    // app_link already destructured from data, fallback if not provided
    const appLink = app_link || 'https://bhishi.app';
    
    const eventMessages: { [key: string]: string } = {
      lucky_draw_started: `🎰 Bhishi Alert! ${adminName || 'Admin'} ne Lucky Draw shuru kar diya hai in ${groupName}! Jaldi aao — live dekhne ka maza lo 🎉 ${appLink}`,
      winner_declared: `🎉 Wah! ${winnerName} ne jeet liya Round ${roundNumber} — ₹${amount?.toLocaleString()}! Badhai ho 👏 Dekho details: ${appLink}`,
      draw_completed: `🏁 Draw complete in ${groupName}! Winner: ${winnerName} — ₹${amount?.toLocaleString()}. Result dekhne ke liye app kholo 🔥 ${appLink}`,
      group_joined: `👋 Welcome to ${groupName}! ${adminName || 'Admin'} ne aapko add kiya. Next draw: ${nextDrawDate || 'TBD'}. Start karne ke liye app kholo 🚀 ${appLink}`,
      payment_reminder: `🔔 Reminder: ${groupName} ka payment ₹${amount?.toLocaleString()} due on ${drawDate || 'soon'}. Jaldi pay karo warna draw miss ho sakta hai — app kholo: ${appLink}`,
      ...
      bid_placed: `⚡ ${bidderName} ne Round ${roundNumber} me ₹${bidAmount?.toLocaleString()} ka bid lagaya in ${groupName}! Ab aapki baari — jaldi app kholo aur compete karo 💪 ${appLink}`,
    };
