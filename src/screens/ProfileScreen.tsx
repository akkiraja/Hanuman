...
import { User, LogOut, Mail, Phone, Calendar, Shield, Settings, Edit3, X, Share2, CreditCard, TestTube } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  const handleShareApp = async () => {
    try {
      const shareMessage = '🚀 Let\'s start a Bhishi(Committee) together! Download the app here: https://committee.short.gy/bhishi';
      
      const result = await Share.share({
        message: shareMessage,
        url: 'https://committee.short.gy/bhishi', // iOS will use this
        title: 'Join My Bhishi App',
      });
  ...
  const handleUpdateName = async () => {
    ...
    // Check against both profile and user metadata
    const currentName = profile?.name || user?.user_metadata?.name || '';
    ...
    console.log('📝 Old name:', user?.user_metadata?.name);
    ...
    try {
      ...
      console.log('✅ === PROFILE NAME UPDATE COMPLETED ===');
      ...
      Alert.alert(
        'Update Failed',
        error.message || 'Failed to update profile name. Please try again.'
      );
    } finally {
  ...
  const handleUpdateUpi = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Update Failed',
        error.message || 'Failed to update UPI ID. Please try again.'
      );
    } finally {
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>🎉 Invite Your Committee Members</Text>
          
          <TouchableOpacity
            style={[styles.inviteButton, Colors.shadow]}
          ...
          >
            ...
            <View style={styles.inviteContent}>
              <Text style={styles.inviteTitle}>📤 Share App</Text>
              <Text style={styles.inviteSubtitle}>
                Invite friends to join your committee
              </Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.section}>
          ...
          {/* Dashboard Testing Button - Re-enabled for payment gateway testing */}
          <TouchableOpacity
