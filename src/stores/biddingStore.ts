  ...
  (set, get) => ({
    ...
    fetchActiveRoundsForUser: async (userId: string) => {
      ...
      try {
        ...
        // For each round, get the current lowest bid from member_bids
        const roundsWithBids = await Promise.all(
          (rounds || []).map(async (round: any) => {
            ...
            return {
              ...
              currentLowestBid: bids && bids.length > 0 ? bids[0].bid_amount : null,
