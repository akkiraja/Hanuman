...
import { ArrowLeft, Users, TrendingDown, Calendar, Trophy, Settings, Trash2, LogOut, X, Crown, Shield, UserPlus, Phone, Search, Check, UserMinus } from 'lucide-react-native';
...
export default function BiddingGroupDetailScreen({ navigation }: BiddingGroupDetailScreenProps) {
  ...
  const { currentGroup, loadGroupDetails, deleteGroup, leaveGroup, addMembersToGroup, appointCoAdmin, removeCoAdmin, removeMemberFromGroup } = useChitStore();
  ...
  // Handle create round with auto-start (fixed 1 hour duration)
  const handleCreateRound = async (prizeAmount: number) => {
  if (!currentGroup?.id) return;

  try {
  // Fixed 1 hour duration
  const biddingHours = 1;
        const endTime = new Date(Date.now() + biddingHours * 60 * 60 * 1000);

  // Calculate minimum bid from custom prize amount
        const minimumBid = Math.floor(prizeAmount * 0.1); // 10% of prize amount
      
      console.log('🎯 Creating round with auto-start...');
      console.log(`⏰ Duration: ${biddingHours} hours`);
      console.log(`💰 Prize: ₹${prizeAmount.toLocaleString()}`);

      // Create round with custom prize amount
      const newRound = await createBidRound(currentGroup.id, endTime, minimumBid, prizeAmount);
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          <View style={styles.addMembersContent}>
            ...
            <View style={styles.addMembersActions}>
              ...
              // Then load contacts (will show loading indicator inside modal)
              await loadContactsWithPermission();
              }}
              >
        ...
        <View style={styles.deleteModalOverlay}>
          <View style={[styles.deleteModal, Colors.shadow]}>
            ...
            <Text style={styles.deleteWarning}>
              This member will be removed and cannot participate in future rounds.
            </Text>
