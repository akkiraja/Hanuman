...
async function sendPaymentReminders(supabaseClient: any, data: PaymentReminderData) {
  try {
    ...
    try {
      console.log(`📱 Attempting to send SMS for payment reminder...`);
      ...
      console.log(`✅ SMS notifications sent successfully for payment reminder`);
    } catch (smsError) {
      console.warn(`⚠️ SMS notification failed for payment reminder, continuing with push notifications:`, smsError.message);
    }
...
async function sendDrawCompletedNotifications(supabaseClient: any, data: DrawCompletedData) {
  try {
    ...
    try {
      console.log(`📱 Attempting to send SMS for lucky draw start...`);
      
      // Get admin name for SMS
      const { data: groupDetails } = await supabaseClient
        .from('bhishi_groups')
        .select('created_by, profiles!inner(name)')
        .eq('id', groupId)
        .single();
      
      await sendSMSNotification({
        groupId,
        event: 'lucky_draw_started', // UPDATED from 'lucky_draw_start'
        groupName,
        adminName: groupDetails?.profiles?.name || 'Admin',
        app_link: 'https://bhishi.app'
      });
      console.log(`✅ SMS notifications sent successfully for lucky draw`);
    } catch (smsError) {
      console.warn(`⚠️ SMS notification failed for lucky draw, continuing with push notifications:`, smsError.message);
    }
...
async function sendGroupJoinedNotifications(supabaseClient: any, data: GroupJoinedData) {
  try {
    ...
    // Also send SMS to new member (with fallback)
    try {
      console.log(`📱 Attempting to send welcome SMS to new member...`);
      
      // Get new member phone number
      ...
      if (memberProfile?.phone) {
        await sendSMSNotification({
          ...
          nextDrawDate: nextDrawDate || 'TBD',
          app_link: 'https://bhishi.app'
        });
        console.log(`✅ Welcome SMS sent successfully to new member`);
      } else {
        console.warn(`⚠️ No phone number found for new member: ${newMemberName}`);
      }
    } catch (smsError) {
...
async function sendBidPlacedNotifications(supabaseClient: any, data: BidPlacedData) {
  try {
    ...
    // Also send SMS notifications (with fallback)
    try {
      console.log(`📱 Attempting to send SMS for bid placed...`);
      await sendSMSNotification({
        groupId,
        event: 'bid_placed',
        groupName,
      ...
      console.log(`✅ SMS notifications sent successfully for bid placed`);
    } catch (smsError) {
      console.warn(`⚠️ SMS notification failed for bid placed, continuing with push notifications:`, smsError.message);
    }
...
async function sendLuckyDrawStartedNotifications(supabaseClient: any, data: LuckyDrawStartedData) {
  try {
    ...
    // Also send SMS notifications (with fallback)
    try {
      console.log(`📱 Attempting to send SMS for lucky draw started...`);
      await sendSMSNotification({
        groupId,
        event: 'lucky_draw_started',
        groupName,
        adminName,
        app_link: 'https://bhishi.app'
      });
      console.log(`✅ SMS notifications sent successfully for lucky draw started`);
    } catch (smsError) {
      console.warn(`⚠️ SMS notification failed for lucky draw started, continuing with push notifications:`, smsError.message);
    }
