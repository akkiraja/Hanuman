...
export const useChitStore = create<ChitStore>()(persist(
  (set, get) => ({
    ...
    finalizeDraw: async (drawId: string) => {
      ...
      try {
        // Update draw record to revealed=true (idempotent)
        const { data: finalizedDraw, error: updateError } = await supabase
          .from('draws')
          .update({ 
            revealed: true,
            status: 'revealed',
        ...
        if (updateError) {
          // Check if already revealed (not an error - idempotent)
          ...
          if (existingDraw && existingDraw.revealed) {
            console.log('✅ Draw already revealed (idempotent), skipping:', existingDraw.id);
            return;
          }
          
          console.error('❌ Failed to finalize draw:', updateError);
          throw new Error('Failed to finalize draw');
        }
    ...
    addMembersToGroup: async (groupId, members) => {
      ...
      try {
        ...
        // Format: [{ userId: 'uuid', name: 'John', phone: '+1234', email: 'john@example.com' }, ...]
        ...
        // Send notifications to newly added active members
        if (addedActive > 0) {
          try {
            ...
            // Get newly added members to send notifications
