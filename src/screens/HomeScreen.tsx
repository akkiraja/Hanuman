  ...
  const checkActiveDraws = async () => {
    ...
    try {
      ...
      if (activeDrawsList.length > 0) {
        ...
        // Only show modal if this draw hasn't been manually dismissed
        if (!dismissedDraws.has(mostRecentDraw.id)) {
          ...
          console.log('🤫 Draw modal dismissed by user, not showing again:', mostRecentDraw.name);
        }
      } else {
                    ...
                    <ModernGroupCard
                      ...
                      // Navigate to appropriate screen based on group type
                        if (group.groupType === 'bidding') {
                          // Prefetch group details for faster load (non-blocking)
                          console.log('🚀 Prefetching group details for:', group.id);
                          loadGroupDetails(group.id).catch(err => 
                            console.warn('Prefetch failed (non-critical):', err)
                          );
